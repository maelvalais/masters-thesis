...
Basis dictionaries were generalized by using multiple bases to form a stable and redundant (also called over-complete) dictionary in \cite{shaobing_chen_atomic_2001}. For example, combining a canonical basis with a cosine basis would allow both peaks and sinusoids to be represented.
...
\begin{itemize}
	...
	\item[--] Rémi Gribonval’s team has proposed a factorization-based dictionary model in the article \citetitle{le_magoarou_flexible_2016} (cf. \cite{le_magoarou_flexible_2016}).
\end{itemize}
...
\Cref{alg_omppalmtree3} suffers from having one more parameter to be chosen: $\alpha$. \Cref{fig_cmp_obj_vs_time} compares the objective function to the time spent for many $\alpha$ from 0 to 70. We did this experiment with both (a) gradient chosen supports and (b) sequentially chosen supports (as described in \cref{sec_pbm_unbalanced_supports}).
