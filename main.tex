...
where the columns of $\Y$ are the vectorized\footnote{A vectorized matrix of dimension $R \times C$ of an image is the concatenation of all columns to form a vector of size $R \cdot C$} images $\y_i \in \R^N$. The dictionary $\D$ is made of $K$ columns $\d_i$ (the \gls{atoms}) such that the number of atoms is much larger than the dimension of a single image $\y_i$ ($K \gg N$):
...
The code $\x$ is a “big” vector of dimension $K$ with $K = N \cdot |\L|$ such that $$\x = \begin{bmatrix}\x^1_1 \\ \vdots \\ \x^{|\L|}_N\end{bmatrix}$$
 ...
 $\D$ is a matrix of dimension $N \times K$ and $\y$ is a vector of dimension $N$.
