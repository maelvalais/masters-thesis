...
\end{subfigure}
\caption{Scattered supports when using plain \cref{alg_omppalmtree}. The gradient in (b) has its maximum values in multiple places, therefore producing scattered supports.} \label{fig_scattered_support}
\end{figure}
...
\begin{algorithm}[!h]
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{Initialization} Initialize the supports $\s^e$ such that $k=1$
    \For{$i = 1,\dots,k$}
      \State $(e,p) = \underset{(e,p) \in \E \times \P}{\argmax}~ |\nabla \Phi_{\h^e}(\h)_p| - \underbrace{\alpha~f(\h,e,p)}_{\text{Regularization for concentrating the support}} $\Comment{Choose support $s^e$ and location $p$} \label{alg_omppalmtree2_find}
      \State $\s^e_p = 1$
...
\includegraphics[width=\textwidth]{figures/tree-learn/gradient-vs-sequential/gradient_not_used.pdf} 
	\caption{}
\end{subfigure}	
\caption{RMSE versus compute time for many $\alpha$. Each graph sums up 35 runs (at $\alpha=0,2,4\dots$) of the \cref{alg_omppalmtree3}.} \label{fig_cmp_obj_vs_time}
\end{figure}

\Cref{alg_omppalmtree3} suffers from having one more parameter to be chosen: $\alpha$. \Cref{fig_cmp_obj_vs_time} compares the RMSE to the time spent for many $\alpha$ from 0 to 70. We did this experiment with both (a) gradient chosen supports and (b) sequentially chosen supports (as described in \cref{sec_pbm_unbalanced_supports}).
