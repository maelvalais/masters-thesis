...
\subsection{The seek for fast transforms}
...
...
Every edge $e$ in $\mathcal{E}$ bears a kernel $h^e$ and a kernel support $s^e$. The figure \ref{fig_example_kernel} exhibits an example of standard kernel filter on the left, and a $h^e$kernel on the right. The kernel support can be understood as the gray squares: they define the locations where the kernel has the right to have values on.



– and one columns of $D$ corresponds to one branch (from the root to one of the leaves) where the kernels are consecutively convoluted.

The dictionary $D$ can then be understood as
...
Keep in mind that $(h^e)_e$ represents the dictionary and is the variable to be minimized when updating the dictionary (see figure \ref{fig_hk_tree}). $x^l$is fixed at all time.
