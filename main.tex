...
Dictionary learning allows us to create “adaptative” dictionaries. Instead of being fixed, the atoms of an “adaptative” dictionary are based on a learning set of example images, denoted by
...
\begin{equation*}\X = \begin{bmatrix} \x_1 & \dots & \x_{|\L|} \end{bmatrix}\end{equation*}
...
\todo{Should I invite the reader to read \cite{rubinstein_dictionaries_2010} which makes a full review of all available transforms (until 2010)?}
...
The code $\x$ is a “big” vector of dimension $K$ with $K = N \cdot |\L|$ such that \begin{equation*}\x = \begin{bmatrix}\x^1_1 \\ \vdots \\ \x^{|\L|}_N\end{bmatrix}.\end{equation*} 
...
In the following, we will denote the images interchangeably using the vector notation  or the linear map notation: \begin{equation*}\y = \{ y_i ~|~ i=1,\dots,N \} \quad \text{is equivalent to}\quad \y = \{y(p) ~|~ p \in \P\}.\end{equation*}
...
\todo{Add an example (image) of what \ac{PALMTREE} is capable of (or more exactly, what it is limited to}
...
\begin{figure}[!h] \centering
\includegraphics[width=0.9\textwidth]{figures/tree-scattered-supports/supports_with_zoom.pdf}
\caption{Scattered supports when using plain \cref{alg_omppalmtree}. The right image is a magnified version of $\s^1$.} \label{fig_scattered_support}
\end{figure}

\Cref{fig_scattered_support} illustrates this issue; every support has its elements scattered instead of being gathered in one place. Ideally, we would like all elements to be centered around the origin (in the figures, the origin is at the center of the image).

\begin{algorithm}[!h]
    \caption{\ac{OMP} algorithm using \ac{PALMTREE} (version 2)}\label{alg_omppalmtree2}
  \begin{algorithmic}[1]
    \Input signal $\y$ of dimension $N$, dictionary of dim $N \times K$ with $K = |\E| \cdot N$ and $K \gg N$
    \Output A $k$-sparse code $\x$ of dimension $K$
    \State \textbf{Initialization} Initialize the supports $\s^e$ such that $k=1$
    \For{$i = 1,\dots,k$}
      \State $(e,p) = \underset{(e,p) \in \E \times \P}{\argmax}~ |\nabla \Phi_{\h^e}(\h)_p| - \alpha \underbrace{\lVert p \rVert_c \cdot \frac{1}{\underset{p'}{\max}\lVert p' \rVert_c}}_{\text{Concentrate the support}} $\Comment{Choose support $s^e$ and location $p$} \label{alg_omppalmtree2_find}
      \State $\s^e_p = 1$
      \State $\h = \underset{\h^e \in \Dspace^e}{\argmin}~ \Phi(\h)$ \Comment{Do $T$ iter. of \ac{PALMTREE}} \label{alg_omppalmtree2_ftl}
    \EndFor
  \end{algorithmic}
\end{algorithm}

To address this issue, we added a new constraint when choosing the new element to be added. \Cref{alg_omppalmtree2} reflects that change. At \cref{alg_omppalmtree2_find}, the term 
\begin{equation}\alpha \underbrace{\lVert p \rVert_c}_{\text{Distance to origin}} \cdot \underbrace{\frac{1}{\underset{p'}{\max}\lVert p' \rVert_c}}_{\text{Distance normalization}}\label{eq_dist_origin}\end{equation}
aims to concentrate each support around its origin. $\lVert p\rVert_c$ is the circular euclidian norm defined on $p=\begin{pmatrix}x \\ y\end{pmatrix}1,\dots,M \times 1,\dots,N$ by

\begin{equation*}\lVert p \rVert_c = \left\| \begin{matrix} \min\left(x \bmod M, (M-x) \bmod M\right) \\ \min\left(y \bmod N, (N-y) \bmod N\right) \end{matrix} \right\|_2\end{equation*}
with $\bmod$ the modulo operator; note that this definition doesn't use the "one dimensional" notation $p \in \P$ previously used because of the circular specificity.


\subsection{Problem of unbalanced supports using \cref{alg_omppalmtree2}} \label{sec_pbm_unbalanced_supports}
Beyond the issue of "scattered" supports, another problem appeared when using the gradient for both choosing $e$. Although The $p$ indicated by the gradient seems appropriate (discussed in \cref{sec_gain_per_added_point}), the maximum value of the gradient for choosing $e$ leads to unbalanced supports: some supports contain most elements, while others have only one or two elements.

\begin{figure}[!h] \centering
\begin{subfigure}[b]{1\textwidth}\centering
\includegraphics[width=0.6\textwidth]{figures/pbm-unbalanced-supp/tree-unbalanced.pdf}
\end{subfigure}
...
We found out that simply adjusting the "concentrating term" \eqref{eq_dist_origin}, with respect to each support, gives much better results. The adjusted "concentrating term" for the modified \cref{alg_omppalmtree3} is
\begin{equation}\alpha \underbrace{\lVert p \rVert_c}_{\text{Distance to origin}} \cdot \underbrace{\frac{\underset{p'}{\max} |\nabla \Phi_{\h^e}(\h)_{p'}|}{\underset{p'}{\max}\lVert p' \rVert_c}}_{\text{Normalized distance (adjusted to each support)}.}\label{eq_dist_origin_adjusted}\end{equation}

\begin{algorithm}[!h]
    \caption{\ac{OMP} algorithm using \ac{PALMTREE} (version 3)}\label{alg_omppalmtree3}
  \begin{algorithmic}[1]
    \Input signal $\y$ of dimension $N$, dictionary of dim $N \times K$ with $K = |\E| \cdot N$ and $K \gg N$
    \Output A $k$-sparse code $\x$ of dimension $K$
    \State \textbf{Initialization} Initialize the supports $\s^e$ such that $k=1$
    \For{$i = 1,\dots,k$}
      \State $(e,p) = \underset{(e,p) \in \E \times \P}{\argmax}~ |\nabla \Phi_{\h^e}(\h)_p| - \alpha \underbrace{\lVert p \rVert_c \cdot \frac{\underset{p'}{\max} |\nabla \Phi_{\h^e}(\h)_{p'}|}{\underset{p'}{\max}\lVert p' \rVert_c}}_{\text{Concentrate support (adjusted to each support)}}$ \label{alg_omppalmtree3_find}
      \State $\s^e_p = 1$
      \State $\h = \underset{\h^e \in \Dspace^e}{\argmin}~ \Phi(\h)$
    \EndFor
  \end{algorithmic}
\end{algorithm}
...
\includegraphics[width=0.8\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad1_every5_add5_totinit0_totadd279_alpha30_tree.pdf}
	\caption{Supports chosen using gradient}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad0_every5_add5_totinit0_totadd279_alpha30_tree.pdf} 
	\caption{Supports chosen sequentially}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad1_every5_add5_totinit0_totadd279_alpha30_approx.pdf}
	\caption{Approximation $\D\x$ of above tree}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad0_every5_add5_totinit0_totadd279_alpha30_approx.pdf} 
	\caption{Approximation $\D\x$ of above tree}
\end{subfigure}
\caption{Comparison of sequential versus using gradient when choosing a support for adding an element.}\label{fig_cmp_sequential_vs_grad}
\end{figure}

To make sure that the support that is chosen using the gradient with this adjusted "concentrating term" \eqref{eq_dist_origin_adjusted} actually gives appropriate results, we compare in \cref{fig_cmp_sequential_vs_grad} the learned supports when 
\begin{enumerate}[label=(\alph*)]
	\item $p$ is chosen using the gradient, $e$ is sequentially selected: we add an element to $\s^1$, then to $\s^2$\dots so that every support has the exact same amount of elements.
	\item $e$ and $p$ are both chosen using the gradient;
\end{enumerate}
We can note that both ways lead to balanced supports. Curiously, choosing the supports sequentially gives better results in terms of RMSE.



\subsection{Choosing the right $\alpha$}
...
\includegraphics[width=\textwidth]{figures/tree-learn/gradient-vs-sequential/gradient_not_used.pdf} 
	\caption{}
\end{subfigure}	
\caption{Objective function versus compute time for many $\alpha$. Each graph sums up 35 runs (at $\alpha=0,2,4\dots$) of the \cref{alg_omppalmtree3}.} \label{fig_cmp_obj_vs_time}
\end{figure}

\Cref{alg_omppalmtree3} suffers from having one more parameter to be chosen: $\alpha$. \Cref{fig_before_after_adding} compares the objective function to the time spent for many $\alpha$ from 0 to 70. We did this experiment with both (a) gradient chosen supports and (b) sequentially chosen supports (as described in \cref{sec_pbm_unbalanced_supports}).

For both (a) and (b), the best $\alpha$ is located in the range from 20 to 30. It is interesting to note that choosing the supports sequentially  significantly reduces the compute time. The objective functions are although similar.

\FloatBarrier
\subsection{Final results using \cref{alg_omppalmtree3}}
\Cref{fig_learnsupp_vs_generic} shows the difference between a tree with generic supports (left) and a learned supports tree (right). Both have the same number of support elements $Q = 279$. Here is some numbers for comparison:
\begin{table}[!h] \centering
\begin{tabular}{@{}lll@{}} \toprule
 & Generic supports (left) & Learned supports (right) \\ \midrule
Initial supports & fixed $3\times3$ supports & diracs \\
RMSE & 0.0102 & 0.0043 (-58\%) \\
Converged after & 358 iterations & 286 iterations \\
Compute time & 87.8 seconds & 89.1 seconds \\ \bottomrule
\end{tabular}
\end{table}

As expected, learning the supports strongly decreases the RMSE. As for the single-branch experiment, the supports are "shaped" into their target atom.

\begin{figure}[!h] \centering
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/xp_learnsupp256_curvelet_decomp3[tree-binary_dpth4]_supp-generic3x3_[fixed-supports]_tree.pdf}
	\caption{Tree with generic supports}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad1_every5_add5_totinit0_totadd279_alpha30_tree.pdf} 
	\caption{Tree with learned supports}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=0.8\textwidth]{figures/tree-learn/xp_learnsupp256_curvelet_decomp3[tree-binary_dpth4]_supp-generic3x3_[fixed-supports]_approx.pdf}
...
\end{subfigure}
\caption{Comparison of generic versus learned supports. Learning the supports allows us to decreases the RMSE by -58\%.}\label{fig_learnsupp_vs_generic}
\end{figure}
...
During this study, we found out that the gradient can be used for learning the support in an \ac{OMP} manner. We also noticed that learning supports on a branch gives excellent results, although being less conclusive on a tree. After some adaptations, the algorithm OMP-PALMTREE gave promising results for curvelets – with the downside of having a new parameter to tune.
