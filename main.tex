...
\subsection{Tuning of regularization parameter $\alpha$}
...
\begin{figure}[!h] \centering
\includegraphics[width=0.6\textwidth]{figures/tree-learn/gradient-vs-sequential/gradient_used.pdf}
	\caption{Tuning of $\alpha$}
\end{figure}

We launched the algorithm for every $\alpha$ ranging from 0 to 70. \Cref{fig_unbalanced_supports} compares the RMSE and compute time with respect to $\alpha$. The optimal range of $\alpha$ seems to be between 20 and 30.
...
\caption{Learned support tree}
\end{subfigure}
\caption{Learned dictionary using \cref{alg_omppalmtree2} with $\alpha=30$.} \label{fig_test_omppalmtree2}
\end{figure}

\Cref{fig_test_omppalmtree2} is the result of \cref{alg_omppalmtree2} for $\alpha=30$. Compared to \cref{fig_test_omppalmtree}, the regularization improves the RMSE by -29\%. It confirms our intuition that “gathered” supports lead to better approximations.
...
We found out that simply adjusting the “concentrating term” \eqref{eq_regularized1}, with respect to each support, gives much better results. The adjusted “concentrating term” for the modified \cref{alg_omppalmtree3} is
\begin{equation}\alpha \underbrace{\lVert p \rVert_c}_{\text{Distance to origin}} \cdot \underbrace{\frac{\underset{p'}{\max} |\nabla \Phi_{\h^e}(\h)_{p'}|}{\underset{p'}{\max}\lVert p' \rVert_c}}_{\text{Normalized distance (adjusted to each support)}.}\label{eq_regularized2}\end{equation}
...
To make sure that the support that is chosen using the gradient with this adjusted “concentrating term” \eqref{eq_regularized2} actually gives appropriate results, we compare in \cref{fig_cmp_sequential_vs_grad} the learned supports when 
