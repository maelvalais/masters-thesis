...
\section{Basis and frames-based dictionaries}
...
More importantly, the Discret Fourier Transform (DFT) has interesting properties that allow fast implementations. For example, applying the Matlab function \texttt{fft2} to the \cref{fig_fourier} takes less than a millisecond. If we assume $N$ to be the dimension of the concatenated\footnotemark[2] image, instead of computing a time-expensive $O(N^2)$ matrix-vector product, the fast Fourier transform only requires a $O(N \log N)$ computational cost.
...
Along with Fourier, many other non-adaptative dictionaries exist; we can mention the cosine transform which is at the core of JPEG compression. We can also cite the wavelet transform, used in JPEG2000 compression (for movies in theaters). Note that both wavelets and cosine are stable and have fast implementations.

Basis-based dictionaries were generalized by using multiple bases to form an over-complete and stable dictionary (called \emph{frame}) in \cite{chen_atomic_2001}. For example, combining a canonical basis with a cosine basis would allow to represent both peaks and sinusoids. The curvelet transform is a good example of such dictionaries; unlike Fourier or cosine transforms, the curvelets can code (to an extent) non-smooth signals.

Although offering excellent performances for sparse representation thanks to the fast implementations, frame-based dictionaries still lacks adaptativity.


\section{Learned dictionaries}

Like fixed-basis frames, learned dictionaries are over-complete. Instead of being fixed, atoms are based on a learning set of example images
$$\Y = \begin{bmatrix} \y_1 & \dots & \y_S \end{bmatrix}$$
where $\Y$ is the concatenation of $S$ column vectors $\y_i$ (the images) of dimension $N$. The dictionary $\D$ is made of $K$ columns $\d_i$ (the atoms) such that the number of atoms is way larger than the dimension of a single image $\y_i$ ($K \gg N$):
...
\end{align}
where $\lVert . \rVert_F$ denotes the Frobenius norm and $\lVert . \rVert_1$ is the $l_1$ norm defined by $\lVert \X \rVert_1 = \sum_{i=1}^S |\x_i|$. This problem can be understood as finding the atoms of $\D$ that give the sparsest (with as many zeros as possible) representation $\x_i$ approximating $\y_i$ for all $i = 1,\dots,S$. This approximation is denoted
...
Because optimizing simultaneously along $\D$ and $\X$ causes non-linearity, many algorithms solving \eqref{eq_dl} alternatively optimize the dictionary $\D$ and the codes $\X$.


\section{The K-SVD algorithm}

Among the many existing alternating algorithms for dictionary learning like the Online dictionary learning algorithm or the K-means algorithm, the \ac{KSVD} algorithm is well known for its state-of-the-art performances in image denoising. Introduced in \cite{aharon_k-svd:_2006}, K-SVD is responsible for many concepts behind the \acs{PALMTREE} algorithm (the algorithm we are trying to improve and developed in \cite{chabiron_optimization_2016}). This section summarizes the key elements of the K-SVD algorithm.
...
The \emph{Sparse Coding step} optimizes along $\x$; this step basically uses \ac{OMP} (see \cref{alg_omp}) or any other pursuit algorithm. The \emph{Dictionary Update step} optimizes along $\D$. This is the step the authors of \cite{chabiron_optimization_2016} worked on. Note that the full K-SVD algorithm is presented in \cref{sec_ksvd_detail}, \cpageref{sec_ksvd_detail}.
...
\end{subfigure}
\caption{Learning $\D$ on image patches using K-SVD.}\label{fig_ksvd}
\end{figure}
...
As the learned K-SVD dictionary relies on the $O(NK)$ matrix-vector product and that it must be over-complete ($K \gg N$) in order to achieve sparsity, we easily understand that $N$ cannot be big. In practice, the example images (also called patches) and atoms of the dictionary are generally at most $16 \times 16$.
...
Translation-invariant transforms are known as convolutional dictionaries. Instead of many atoms containing translated version of the same feature, convolutional dictionaries contain all possible translations for every atoms; unlike traditional learned dictionaries, the atoms are more likely to be unique.
...
\todo[inline]{Didn't I say everything in \cref{sec_atoms_redund}?}

\section{The convolutional tree algorithm}

\subsection{Convolutional tree model \label{sec_tree_model}}
...
The target image $\y$ as well as the kernels $\h^e$ and supports $\s^e$ are vectors of dimension $N$. The supports $\s^e$ only take the values 0 (corresponding value in $\h^e$ must be 0) or 1 (corresponding value in $\h^e$ can be non-zero). The code $\x$ is a "big" vector of dimension $K$ with $K = N \times Leaves$; $\D$ is a matrix of dimensions $N \times K$ and $\y$ is a vector of dimension $N$.

In the following, we will denote the image interchangeably using the vector notation $y = \{ y_i ~|~ i=1,\dots,N \}$ or the linear map notation $y = \{y(p) ~|~ p \in \P\}$.
...
The dictionary $\D$ can be written as a matrix (showed in \cref{sec_matrix_vs_tree}). This allows us to compare the computational complexity of this dictionary against a simple matrix-vector product. The convolutional tree dictionary computes $\D\x$ using the following

\begin{align}
	\D\x = \sum_{l \in \L} \x^l * \h^{*l}	 \label{eq_Dx_as_convolution}
\end{align}

The product $\D\x$ can be seen as a polynomial of degree $2 \times \text{depth}(\T)$. This implies that it is possibly strongly non-convex but infinitely differentiable.
...
The convolution of sparse kernels in \eqref{eq_ftl} leads to theoretically extremely performant dictionaries. The computational cost of $\D\x$ is reduced\footnote{This is the simplified complexity. See \cref{sec_palmtree} for the actual complexity.} to $$O(|\E| N log N)$$ with $|\E|$ the number of edges; and because $|\E|$ is significantly smaller than $N$, this is very close to the performance of the fast Fourier transform.

However, $\D\x$ is possibly extremely non-convex (as mentioned in \cref{sec_tree_model}), suggesting that the optimization of \eqref{eq_ftl} is vain and that we would end up trapped by its many suboptimal critical points. Beyond proving that this problem is actually tractable, the authors of \cite{chabiron_optimization_2016} made the fascinating discovery that optimizing \eqref{eq_ftl} is possible and that the solutions given by \acs{PALMTREE} were encouraging.
...
\begin{algorithm}[!ht]
    \caption{\ac{PALMTREE} (Proximal Alternating Linearized Minimization for Convolutional Tree model) algorithm for Dictionary Update}\label{alg_palmtree}
  \begin{algorithmic}[1]
    \Input
    \begin{itemize}
    	\item[--] image $\y$
...
As described in \cref{sec_tree_model}, the supports $\s^e$ are part of the convolutional tree model. In \cite{chabiron_toward_2015}, the authors experimentally showed that using fixed supports for approximating is possible many kinds of atoms (curvelets, cosines). For the specific purpose of approximating existing atoms, the results turned out to be very promising. However, the proposed model was supposed to be able to approximate any kind of image.
...
The Orthogonal Matching Pursuit (\ac{OMP}, detailed in \cref{alg_omp}) chooses which component $\x_i$ should be "turned on" at each iteration, finding the atom $d_i$ that has the greatest correlation with the residual $R$. This step (cf. \cref{alg_omp_pick_correlation}) uses the gradient $\nabla \Phi(\x)$, with $$\Phi(\x) = \lVert \D\x - \y \rVert^2$$
...
Our idea is to use the same principle when adding elements to supports $(\s^e)_{e \in \E}$: add a new element to one of the supports between two iterations of the \ac{PALMTREE} algorithm, using the greatest gradient component information.
...
\begin{align*}
	\nabla_{\h^e} \Phi((\h^f)_{f \in \E})&= \F^{-1}(\F(\H^{e})^* \text{.\^{}} \F(\Res))
\end{align*}
with ${}^*$ denoting the adjoint operator, $\text{.\^{}}$ the point-wise product; $\H^{e}$ as well as $\Res$ are notations defined in \cref{sec_palmtree}. % TODO H^e?
...
$\g^f$ represents for each point $p \in \P$ the minimized objective function if an element were added on kernel support $\s^f$ at that specific location (cf. \cref{alg_gain_per_added_point}).
...
\end{subfigure}
\end{subfigure}
\caption{Minimization of a single branch tree with fixed supports. Note the zeros of the gradient on each support location (small squares): in the direction of those nine points, the gradient is null, meaning that we are at a critical point.}\label{fig_gain_n4}
\end{figure}


\Cref{fig_gain_n1_n2} makes the same comparison for two other kernels of the same tree (it is the same experiment). Whatever the kernel, the best values of the gradient seem to follow the same shape as the gain-per-added-point matrix. 
...
The experiment shown in \cref{fig_before_after_adding} is based on the results of \cref{fig_gain_n4_approx}, where figures (a,b,d,e,g,h) are the same as the previous experiment. The figures (c,j,i) show what  happens if we add the minimum point of gain-per-added-point to $\s^4$ and continue the minimization. We notice the approximation (c) looks visually closer to the target image; the RMSE is decreased by -16\% (lower is better).
...
\subsection{Local versus distant information}

The gain-per-added-point can been seen as a distant direction (each point is the objective function of a converged solution) whereas the gradient is a local direction. The  \cref{fig_gain_n4} also presents the gradient of a converged solution, and we noticed that it had the same shape as gain-per-added-point. We might wonder if this only happens on converged solutions or if the gradient also give a valid information whatever the iteration.

% meaning that the gain-per-added-point would be "close" to what the gradient would indicate. We wondered if the gradient would stay the same throughout all iterations; compared to the converged gain-per-added-point which is a "distant" information, the gradient might not always indicate the direction to that gain.

With this problem in mind, \cref{fig_iter_gain_vs_grad} compares the best values of gain-per-added-point to gradients snapshots taken during the minimization. This is reassuring: whatever the iteration, the gradient best values keep close to the gain-per-added-point best values.
...
\begin{subfigure}[b]{0.49\linewidth}\centering
	...
	\end{subfigure}
\end{subfigure}
\caption{Is the gradient giving the same information depending on the iteration? The conditions are the same as \cref{fig_gain_n4}. Only the best values are displayed.}\label{fig_iter_gain_vs_grad}
\end{figure}
...
After having shown that the gradient would give a good indication on which support elements should be added, we made our first experiments of learning the support.

In \cref{fig_variable_support}, the initial supports are simply centered Diracs; every five iterations, one element is added where the gradient is the greatest, up to 36 elements. The experiment converges after about 200 iterations. 
...
\end{subfigure}
\caption{Learning the support (add only). Starting from simple Diracs, the best element (using greatest gradient) is added to one of the supports every 5 iterations, up to 36 elements (same number as the fixed support in \cref{fig_gain_n4}).} \label{fig_variable_support}
\end{figure}
...
Figure \ref{fig_block_circular} outlines the relation between the tree and the matrix forms of $\D\x$ in the convolutional tree model. The dictionary $\D$ can be written using the notation
...
$$\y=\hat{\y} + \bm{b}$$
with $b$ following a centered Gaussian distribution $\mathcal{N}(0,\sigma^2I)$ and $\hat{\y}$ the noise-less signal. We see that the distance $||b||$ is always smaller than the projected distance.
