...
with $L$ the number of leaves in the tree – $l$ means "leaf"–. Each atom is thus written as (with the example of figure \ref{fig_matrix_vs_tree}) :
\begin{align*}
H^{l_4} = h^1 * h^2 \dots * h^k
\end{align*}


\begin{figure}[!h] \centering
\includegraphics[width=0.9\textwidth]{figures/matrix-vs-tree.pdf} \caption{ \label{fig_matrix_vs_tree}}
\end{figure}


a theoretical framework to the (FTL) problem. The (FTL) problem aims at approximating an ideal dictionary $D^{ideal}$ (which can be easily understood as a matrix) made of ideal atoms $a_i$ on its columns. The approximated dictionary will be called $D$ and the atoms formed by its columns $(H^f)_{f \in \leaves}$. 
