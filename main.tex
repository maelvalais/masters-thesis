...
In the following, we will denote interchangeably the image using the vector notation $y = \{ y_i ~|~ i=1,\dots,N \}$ or the linear map notation $y = \{y(p) ~|~ p \in \P\}$.
...
The convolution of sparse kernels in \eqref{eq_ftl} leads to theoretically extremely performant dictionaries. The computational cost of $\D\x$ is reduced\footnote{This is the simplified complexity. See \cref{sec_palmtree} for the actual complexity.} to $$O(|\E| N log N)$$ with $|\E|$ the number of edges; this is very close to the performance of the fast Fourier transform.
...
This chapter covers the protocols and results of the various experiments aimed to learning the supports during the PALMTREE algorithm.
