...
In the following, we will denote the images interchangeably using the vector notation  or the linear map notation: $$\y = \{ y_i ~|~ i=1,\dots,N \} \quad \text{is equivalent to}\quad \y = \{y(p) ~|~ p \in \P\}.$$
...
\footnotetext[1]{Let $\y^1$ and $\y^2 \in \R^N$; the Root Mean Square Error is defined by  $$\text{RMSE}(\y^1,\y^2) = \frac{1}{N} \sqrt{\sum_{i=1}^N (y^1_i - y^2_i)^2}$$}
...
\subsection{Learning supports for a single branch}
...
\subsection{Learning supports for a tree}

\Cref{alg_omppalmtree} gives encouraging results; however, two issues rose when trying the same algorithm on a tree:
\begin{itemize}
	\item[-] the gradient indicates elements that are far from each other, hence making the kernels extremely spread;
	\item[-] even though all points are gathered, the gradient might sometimes lead to very large kernels.
\end{itemize}

To address these issues, we added a new constraint when choosing the new element to be added. \Cref{alg_omppalmtree2} reflects that change. At \cref{alg_omppalmtree2_find}, the term 
$$\alpha \underbrace{\lVert p \rVert_C}_{\text{Distance to origin}} \cdot \underbrace{\frac{\underset{p'}{\max} |\nabla \Phi_{\h^e}(\h)_{p'}|}{\underset{p'}{\max}\lVert p' \rVert_C}}_{\text{Normalization}}$$
aims to concentrate the supports around the origin. $\lVert p\rVert_C$ is the circular euclidian norm, defined by
$$\lVert p \rVert_C = \lVert \min\left(p ~\text{mod}~ N, (N-p) ~\text{mod}~ N\right) \rVert_2$$

\begin{itemize}
	\item max gradient seems to always lead to adding on the root more than on others
	\item adding multiple elmts every ~5 iterations of PALMTREE gives good results
	\item  
\end{itemize}
...
\begin{algorithm}[!ht]
  ...
  \begin{algorithmic}[1]
    ...
    \State \textbf{Initialization} Initialize the supports $\s^e$ such that $k=1$
    \For{$i = 1,\dots,k$}
      \State $(e,p) = \underset{(e,p) \in \E \times \P}{\argmax}~ |\nabla \Phi_{\h^e}(\h)_p| - \alpha \underbrace{\lVert p \rVert_C \cdot \frac{\underset{p'}{\max} |\nabla \Phi_{\h^e}(\h)_{p'}|}{\underset{p'}{\max}\lVert p' \rVert_C}}_{\text{Concentrate the support}} $\Comment{Choose support $s^e$ and location $p$} \label{alg_omppalmtree2_find}
      \State $\s^e_p = 1$
...
\includegraphics[width=0.5\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad1_every5_add5_totinit0_totadd279_alpha30_target.pdf}
\caption{The target $\y$ made of curvelets at different scales and angles}
\end{figure}
...
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad0_every5_add5_totinit0_totadd279_alpha30_approx.pdf} 
	\caption{}
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad1_every5_add5_totinit0_totadd279_alpha30_tree.pdf}
...
\end{subfigure}
\begin{subfigure}[b]{0.49\textwidth}\centering
\includegraphics[width=\textwidth]{figures/tree-learn/gradient-vs-sequential/xp_learnsupp256_curvelet_decomp3_tree-binary_dpth4_supp-diracs_usegrad0_every5_add5_totinit0_totadd279_alpha30_tree.pdf} 
	\caption{}
\end{subfigure}
\caption{}
\end{figure}
