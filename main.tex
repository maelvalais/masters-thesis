...
\end{subfigure}
\caption{Results after using \cref{alg_omppalmtree}. The “range” of the elements is given under each support. For example, with $\s^1$ (middle support), the number 141 means that all elements fit in a square of width 141.}\label{fig_test_omppalmtree}
\end{figure}\todo{tell the size of each square}

\noindent
A first result using the original OMP-PALMTREE (\cref{alg_omppalmtree}) is displayed in \Cref{fig_test_omppalmtree}. Note that we cannot distinguish the elements of the supports. A zoom on one of the supports is made in \cref{fig_scattered_support_tree} (section below).
